...
For debugging purposes, it would be good to set expectations about which service is running on which port.
...
- I got no feedback from the wallet when deploying a program - had to chase sequencer logs to see if there was a transaction on the chain and the program was deployed properly. Ideally a program ID or address or similar would be displayed, possibly also to be used for searching for the program via the explore.
