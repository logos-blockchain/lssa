...
pub fn get_associated_token_account_id(
    ata_program_id: &ProgramId,
    owner_id: AccountId,
    definition_id: AccountId,
) -> AccountId {
    AccountId::from((ata_program_id, &compute_ata_seed(owner_id, definition_id)))
}
...
pub fn verify_ata_and_get_seed(
    ...
    let expected_id =
        get_associated_token_account_id(&ata_program_id, owner.account_id, definition_id);
    assert_eq!(
